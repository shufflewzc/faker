...
远程-重写获取Cookie
https://raw.githubusercontent.com/shufflewzc/faker/main/conf/zhengjia.conf
-----------------------------------------------
...
class UserInfo {
    ...
    async signin() {
        try {
            // 构建请求参数
            const options = {
                url: `https://fzcrmapi.china-plaza.cn:7002/ApiMember/api/VipInfo/Sign`,
                headers: {
    ...
    async getUserId() {
        try {
            const options = {
                url: `https://fzcrmapi.china-plaza.cn:7002/ApiMember/api/VipInfo/QueryVipInfoAsync`,
                headers: {
                    'Content-Type': 'application/json',
                    'User-Agent':
            ...
            const { data, success ,msg } = (await httpRequest(options)) ?? {}
            if (success === true) {
                this.userId = data?.member_surname
                console.log(`用户名: ${data.member_surname}`)
            } else {
...
async function getCookie() {
    ...
    ;(async () => {
        if ($.isReuqest()) {
            const { Authorization: tokenValue } = ObjectKeys2LowerCase($request.headers)
            if (tokenValue) {
