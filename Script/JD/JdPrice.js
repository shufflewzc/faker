...
function lowerMsgs(data) {
    ...
    const lowerMsg = "Faker提示您：历史最低价格：¥" + String(lower) + ` (${lowerDate}) `
    return lowerMsg
}
    ...
    list.forEach((item, index) => {
        if (item.length > 0) {
            ...
            if (index == 0) {
                ...
                lowest180 = { Name: "半年最低", Price: `¥${String(price)}`, Date: date, Difference: difference(currentPrice, price), price }
                lowest360 = { Name: "一年最低", Price: `¥${String(price)}`, Date: date, Difference: difference(currentPrice, price), price }
            }
